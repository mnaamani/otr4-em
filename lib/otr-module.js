...
function User( config ){
  ...
  //use files in VFS specified in config -
  //files from real file must be imported before loading them here
  if(config && config.keys && config.fingerprints && config.instags){
    ...
    //no files specified, use auto generated file names
    this.keys = nextVfsFileName();
    this.fingerprints = nextVfsFileName();
    this.instags = nextVfsFileName();
  }
...
User.prototype.writeKeysToFS = function(filename,transform){
  otr.VFS().exportFile(this.keys,filename,transform);
};

User.prototype.writeFingerprintsToFS = function(filename,transform){
  otr.VFS().exportFile(this.fingerprints,filename,transform);
};

User.prototype.writeInstagsToFS = function(filename,transform){
