...
function User(files){
  ...
  if(files){
    ...
    if(files.instags){
      ...
      }catch(e){ console.error("Warning Reading Instags:",e);}
    }
  }
}
...
function Account(user,accountname,protocol){
  ...
  this.generateKey = function(callback){
    user.state.generateKey(user.keys,accountname,protocol,function(){
      if(callback) callback.apply(account,arguments);
    });
  };
  ...
  this.generateInstag = function(callback){
    try{
      ...
      if(callback) callback(null, user.state.findInstag(accountname,protocol));
    }catch(e){
  ...
  this.contacts = function(){
    ...
    contexts.forEach(function(context){
      if(context.their_instance() !== 0) return;
...
function Contact(user,accountname,protocol,recipient){
    ...
    this.openSession = function(parameters){
      return new Session(user,new otr.ConnContext(user.state,accountname,protocol,recipient),parameters);
    };
