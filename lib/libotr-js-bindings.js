...
OtrlUserState.prototype.generateInstags = function (filename,accountname,protocol){
   if(typeof filename == 'string'){    
        var err = otrl_.instag_generate(this._pointer,filename, accountname,protocol);
        return (err?gcry_.strerror(err):undefined);
    }else{
        throw("invalid arguments to readInstags()");
    }
};
//ConnContext
...
OtrlMessageAppOps.prototype.messageSending = function(userstate,accountname,protocol,recipient,message, otrchannel){
    if(!(
