...
OtrlMessageAppOps.prototype.extraSymKey = function(userstate,context,use,usedata){
    //return Buffer(32) (256bit extra symmetric key)

    var symkey_ptr = _malloc(32);//OTRL_EXTRAKEY_BYTES
    var symkey;

    if(typeof usedata == 'string'){
        usedata = new Buffer(usedata);
    }
    var usedata_ptr = _malloc(usedata.length);
    for(var i=0; i<usedata.length; i++){
        setValue(usedata_ptr+i,usedata.readInt8(i),"i8");
    }

    var err = otrl_.message_symkey(userstate._pointer,this._pointer,this._opsdata,context._pointer,use, usedata_ptr, usedata.length, symkey_ptr);

    if(!err){
        symkey = helper_.ptr_to_Buffer(symkey_ptr,32);
    }
